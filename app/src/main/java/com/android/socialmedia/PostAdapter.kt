...
class PostAdapter(options: FirestoreRecyclerOptions<Post>, val listener: MainActivity) : FirestoreRecyclerAdapter<Post, PostAdapter.PostViewHolder>(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PostViewHolder {
        val viewHolder =  PostViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.item_list, parent, false))
    ...
    override fun onBindViewHolder(holder: PostViewHolder, position: Int, model: Post) {
        ...
        holder.userText.text = model.creator.displayName
        Glide.with(holder.userImage.context).load(model.creator.imageUrl).circleCrop().into(holder.userImage)
        holder.likeCount.text = model.likedBy!!.size.toString()
        holder.createdAt.text = Utils.getTimeAgo(model.timeAt)
